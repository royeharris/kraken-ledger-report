# Kraken Ledger Report — Technical Notes (vv3.045)
...
## Statement Summary additions (v3.045)
- New section appended to Statement Summary: **Reconciliation Difference**.
- Reconciliation is computed in GBP terms only:
  - Use raw GBP values for GBP lines.
  - Use GBP-equivalent values for non-GBP assets (e.g., USDT).
- Display:
  - One-row table (or single-value row) with the GBP difference.
  - Followed by the fixed explanatory sentence.
